...
var compito = {
    studia() {
        console.log(`Per favore studia ${ this.argomento }`);
    }
};

var compitoJS = Object.create(compito);
compitoJS.argomento = "JS";
compitoJS.studia();
// Per favore studia JS

var compitoMatematica = Object.create(compito);
compitoMatematica.argomento = "Matematica";
compitoMatematica.studia();
// Per favore studia Matematica
```

I due oggetti `compitoJS` e `compitoMatematica` sono entrambi collegati tramite prototipo all'unico oggetto compito, che ha la funzione `studia()`. A ciascuno di essi viene assegnata la propria proprietà argomento (vedi Figura 3).
